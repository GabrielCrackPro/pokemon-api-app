...
const showPokemon = async (query) => {
    ...
    const pokemonData = {
        ...
        /*TODO: FIX THIS 
        moves: {
            firstMove: pokemon.moves[0].move.name,
            secondMove: pokemon.moves[1].move.name,
            thirdMovement: pokemon.moves[2].move.name,
            fourthMovement: pokemon.moves[3].move.name
        },*/
    ...
    <div class="border border-3 border-dark rounded-circle mb-2">
    <img src="${pokemonData.img}" height="100" width="100" alt="front_sprite" class="front-img"></img>
    </div>
    <div class="container border border-3 border-dark w-50 text-center">
    <p class="fs-3 fw-bold text-uppercase border-bottom border-dark border-3">Base Stats</p>
